...
def main(args):
    ...
    for ino in range(len(img_list)):
        # 获取图像文件名和预测结果
        image_file = valid_image_file_list[ino]
        pred = rec_res[ino]
        ...
        if match:
            number = match.group()
            result_str += '{} {}\n'.format(number, pred[0])
        # 将文件名和预测结果添加到字符串中

        #result_str += '{}\t{}\n'.format(image_file, pred[0])
    # # 打印最终的字符串
    # print(result_str)
    # 指定要保存的文件名
    result_file = r'C:\Users\Alex\PycharmProjects\Table_rec_system\Table_detection\results\Identification_res.txt'
    # 打开文件并写入字符串
